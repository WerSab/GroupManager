...
import { getCollection, getDocumentReferenceById, getFirestoreTimestampFromMillis } from '../fireBase/firestore-Helper';
...
export const setNewCleanUpDate = (presentDate) => {
    return new Promise((resolve) => {
        ...
        AsyncStorage.setItem('ticketLatestCleanUpDate', jsonPresentDate)
            .then(() => {
                resolve();
            })
    })
...
const deleteOldUserTickets = (userId, nowMillis) => {
    ...
    return new Promise((reject) => {
        const timestampFromCurrentDate = getFirestoreTimestampFromMillis(nowMillis - THREE_DAYS_MILLIS);
        ...
        getCollection(FIRESTORE_COLLECTION.TICKETS)
            ...
            .then(querySnapshot => {
                ...
                bulkDeleteTicket(documentReferences);
            })
